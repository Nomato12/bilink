...
class ServiceRequestCard extends StatelessWidget {
  ...
  // Add these getters to access status and serviceType throughout the class
  String get status => requestData['status'] ?? 'pending';  String get serviceType {
    // أولاً: فحص وجود نوع خدمة محدد بشكل صريح
    String type = requestData['serviceType'] ?? '';
    if (type.isEmpty) {
      type = requestData['type'] ?? '';
    }
    
    // إذا كان النوع لا يزال فارغًا، حدد بناء على خصائص طلب النقل
    if (type.isEmpty) {
      final bool hasOriginLocation = requestData['originLocation'] != null;
      final bool hasDestinationLocation = requestData['destinationLocation'] != null;
      final bool hasVehicleType = requestData['vehicleType'] != null && 
          requestData['vehicleType'].toString().isNotEmpty;
      
      // إذا كانت تحتوي على موقع انطلاق ووصول أو نوع مركبة، فهي خدمة نقل
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      ),                      // Show service type indicator - fixed to ensure correct display
                      Container(
                        ...
                        child: Text(
                          // Ensure only one service type is shown
...
void _updateRequestStatus(BuildContext context, String newStatus) async {
  ...
  try {
    ...
    if (clientId.isNotEmpty) {
      ...
      try {
        ...
        try {          await FirebaseFirestore.instance.collection('notifications').add({
            'userId': clientId,
            'title': title,
            'body': body,
            'isRead': false,
            ...
            'data': {
              ...
              'serviceType': serviceType, // Use the same serviceType variable determined earlier
  ...
  void _showFullMap(BuildContext context) {
    // استخدام المتغير serviceType بدلا من قراءة القيمة مباشرة من قاموس البيانات
    if (serviceType == 'نقل') {
      final GeoPoint? originLocation = requestData['originLocation'] as GeoPoint?;
      final GeoPoint? destinationLocation = requestData['destinationLocation'] as GeoPoint?;
